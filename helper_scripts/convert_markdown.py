"""
Markdown to HTML conversion helper functions for Field Notes

This module provides functions to convert markdown files to HTML using Pandoc.
"""
...
def convert_markdown_to_html_with_pandoc(markdown_file, html_output_file):
    ...
    Args:
        markdown_file: Path to source markdown file (str or Path)
        html_output_file: Path to output HTML file (str or Path)
...
def convert_all_markdown_files():
    ...
    for markdown_file in markdown_dir.rglob('*.md'):
        ...
        if convert_markdown_to_html_with_pandoc(str(markdown_file), str(html_output_path)):
            converted_count += 1
